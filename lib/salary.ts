...
//
// 🔴 2026-08-05 티켓 #35 정정: 근로소득세액공제 한도(STEP 7)의 분기가 3단뿐이라
//   **제59조제2항제4호(총급여 1억2천만원 초과)가 구현돼 있지 않았다.** 종전 코드는
//   마지막 `else`가 7천만원 초과를 전부 삼켜, 제3호의 하한 50만원을 1억2천만원 초과
//   구간까지 그대로 연장 적용했다. 제4호는 1억2천만원 초과분에 대해 50만원에서
//   다시 1/2씩 체감하고 하한이 20만원이므로 한도가 최대 연 30만원 과대였고,
//   근로소득세 월 25,000원·지방소득세 월 2,500원 과소 → 월 실수령액 최대
//   27,500원 과대 표시(연 33만원)였다. 사용자에게 유리한 방향의 오차라
//   신고가 들어오지 않는 유형이다.
//   ⚠️ 영향 구간은 "1억2,060만원 초과"가 **아니다**(2026-08-05 QA step=1 실측 정정).
//     정확히는 ① 표시값이 처음 갈리는 G = **120,000,002**, ② 120,000,015까지는
//     `floor(DT/12)` 양자화가 격차를 삼켰다 뱉는 **톱니 구간**이라 단일 임계로
//     서술할 수 없으며, ③ 격차는 0에서 선형 증가해 G = **120,599,983에서 27,500원에
//     도달**하고 120,599,997부터 영구 고정된다. 즉 최대 영향의 시작은 "초과"가 아니라
//     **"이상"**이고, 1.2억~1.206억은 부분 영향 램프 구간이다. 경계 부등호를 틀린 것이
//     이 티켓의 결함 자체였으므로 그 정정문에서 같은 실수를 반복하지 않도록 명시한다.
//   근거: 소득세법 제59조제2항제4호. 2026-08-05 law.go.kr MST 280405(시행 2026-07-01)
//   조문 대조. 4개 호 ↔ 4개 분기가 1:1 대응하도록 각 분기에 조문 포인터 주석을 달았다.
//   경계·하한 앵커는 `scripts/regression/engines.test.ts`가 잠근다.
// =============================================================================
...
function calculateIncomeTax(
  ...
  //         한도는 소득세법 제59조제2항 4개 호 그대로. 각 분기 = 각 호.
  ...
  } else if (G <= 120_000_000) {
    // 제59조제2항제3호
    creditCap = Math.max(500_000, 660_000 - (G - 70_000_000) * 0.5);
  } else {
    // 제59조제2항제4호
    creditCap = Math.max(200_000, 500_000 - (G - 120_000_000) * 0.5);
  }
  ...
  // STEP 11  지방소득세(월) — 절사
  //
  // ⚠️ 2026-08-05 티켓 #38 조사 결과 — 이 1원 단위 `Math.floor`에는 조문 근거가 없다.
  //   「지방세기본법」 제59조는 「국고금 관리법」 **제47조(전체)**를 **강행 준용**하며("준용한다"),
  //   그중 **제47조제1항**이 **10원 미만 버림**을 정한다. 1원 절사는 「국고금 관리법」
  //   **제47조제2항**인데 그쪽은 **과세표준액 산정 전용**이라 세액 산정인 여기에는 걸리지 않는다.
  //   (준용 목적어를 "제47조제1항"으로 좁혀 적지 말 것 — 제59조는 항을 특정하지 않는다. 표준 ㋑)
  //   그런데도 10원 절사로 바꾸지 않은 이유:
  //   STEP 10이 간이세액표(소득세법 제134조제1항)가 아니라 **연 결정세액 ÷ 12 근사**여서
  //   여기서 나오는 값은 애초에 법정 "매월분 원천징수세액"이 아니다. 자기 오차가 수천 원인
  //   값의 1의 자리에 법정 절사를 입히면 정확도는 그대로면서 **10원 단위라는 형태 신호만
  //   위조**된다. 즉 절사 단위는 간이세액표를 구현한 뒤에 그 값 위에서 정해야 하는 종속 문제다.
  //   👉 간이세액표 구현 시 STEP 10·11의 절사 단위를 **함께** 결정할 것.
  //   조사·판정 전문: `planning/local-income-tax-rounding-research.md` (마스터 판정 블록).
  //   ※ 승수 0.1 자체는 정본 일치 확인됨 — 지방세법 제103조의13제1항 "100분의 10".
