...
//   - **주휴수당 2편**(guide·eligibility) title 속 연도는 `${MINIMUM_WAGE_YEAR}` 보간이라
//     상수 교체만으로 따라온다(티켓 #43). 자동인 것은 **그 2편 title의 연도 하나뿐**이며,
//     같은 2편의 slug도, 다른 글의 title도 아니다(아래 (a) 참조).
...
//   (a) **주휴수당 2편**(guide·eligibility)의 slug 끝에 박힌 연도 꼬리표
//       → 연도 브랜딩이 박힌 URL이라 **동결이 정답일 수 있다**. 갱신하면 기존 URL의
//         색인·링크를 버리는 비용이 붙으므로 기획 판단 대상이지 자동화 대상이 아니다.
//         (동결을 택하면 slug의 연도만 본문·title보다 뒤처진 상태로 남는다. 그건
//          의도된 잔존이지 누락이 아니라는 판단을 그 회차에 명시적으로 남길 것.)
//       ※ **title은 (a) 소관이 아니다.** 위 자동 갱신 목록대로 보간이라 손댈 필요가
//         없다(티켓 #43). 교체 회차에 title을 리터럴 연도로 되돌리지 말 것 —
//         되돌리는 순간 이 항목이 다시 수동 대상으로 늘어난다.
//       ※ 연도가 박힌 slug·title은 이 2편 말고도 블로그 전반에 있다. 그 전수 현황과
//         처리 정책(동결/갱신)은 최저시급과 무관한 글까지 포함하므로 이 상수에 묶지
//         않고 **티켓 #42(연도 표기 정책)에서 별도로 관리한다**. 여기에 건수를 적어
//         두면 글이 늘거나 상수를 교체할 때마다 썩으므로 숫자는 적지 않는다.
