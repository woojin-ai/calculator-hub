...
필요한 기술·콘텐츠 요건은 사실상 충족**된 상태로 판단됩니다. 남은 것은 **(1) 실배포 URL이 실제로
살아있는지 최종 확인**과 **(2) 사용자의 신청 결정**뿐입니다. 승인 자체는 Google 심사 소관이라
보장할 수 없습니다.
...
1. **실배포 URL 생존 확인 (사용자/감사 트랙)** — `lib/site.ts`의 `SITE_URL`
   = `https://calculator-hub-delta.vercel.app`. 이 세션은 헤드리스라 외부 URL 실접속을 확정하지
   못함. AdSense 심사는 **실제 살아있는 URL**을 크롤링하므로, 신청 전 이 주소가 200으로 열리는지
   사용자가 확인 필요. (커스텀 도메인을 붙일 계획이라면 신청 전에 연결하는 편이 유리)
...
- **(a) AdSense 신청 진입** — 위 1번(배포 URL 생존)만 확인되면 지금이 적기. 승인 후 ads.txt +
  광고 스크립트 삽입은 팀이 처리 가능.
