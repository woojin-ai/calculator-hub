# 블로그 목록 — 페이지네이션 + 검색 UI 구성안 (v1.1, 2026-07-17)

> **[v1.1 갱신]** 배포 당일 사용자가 §1 결정1(그리드 전환)을 직접 반려 — **§11 "카드 레이아웃 되돌림"** 신규 추가. 페이지네이션/검색(§2~§3)은 영향 없음, 그대로 유효.
...
>
> **[2026-07-17 갱신] 실제로 회수됨.** 배포 당일 사용자가 실사용 후 "이전 단일 컬럼 리스트가 더 좋았다"고 직접 반려 — 원인은 카드 폭이 좁아져 제목 `line-clamp-2`가 훨씬 이른 글자 수에서 걸린 것(§11-2 실측). **이 "결정 1"은 §11에서 되돌려졌다.** 판단 기준(기존 패턴 재사용 = 톤앤매너 변경 아님) 자체는 여전히 합리적이었지만, 텍스트가 주가 되는 카드에는 좁은 컬럼이 가독성 문제를 일으킬 수 있다는 게 이번에 확인된 셈 — 근거는 남겨두고 결론만 §11로 덮어쓴다.
...
- [ ] ~~데스크톱(1024px+) 3열, 태블릿(640~1023px) 2열, 모바일(375px) 1열 그리드 확인.~~ → **§11에서 되돌림, 무효화됨.** 대신 §11-5 체크리스트(전 구간 단일 컬럼 확인) 사용.
...
| 컨테이너/그리드 | ~~`max-w-3xl` 스택 → `max-w-5xl` 그리드~~ → **§11에서 `max-w-3xl` 단일 컬럼으로 재되돌림** | 배포 당일 사용자 반려. 최종 확정 값은 §11-1 참고 |
...
## 11. 카드 레이아웃 되돌림 — 2026-07-17 사용자 피드백

### 11-0. 계기 · 원인 · 되돌림 범위

**계기**: 오늘 배포된 3열 그리드(§1 결정1)를 마스터가 실사용 후 사용자 반응 그대로 전달 — "이전 디자인(한 줄씩 길게, 단일 컬럼 리스트)이 더 좋았다"고 명확히 반려.

**원인**: `BlogPostCard.tsx`의 제목 `line-clamp-2`는 그리드 도입 전후로 전혀 바뀌지 않았다. 바뀐 건 카드가 담기는 컬럼 폭뿐이다 — 데스크톱 기준 단일 컬럼일 때 카드 내부 텍스트 폭은 약 **688px**였는데, 3열 그리드에서는 약 **283px**로 좁아졌다(계산 근거는 §11-2). 같은 "2줄까지" 규칙이 폭이 2.4배 좁아진 상태에서 적용되니 훨씬 이른 글자 수에서 걸리게 됐고, 그 결과가 마스터가 전달한 예시(48자 제목 "겨울 전기요금이 여름보다 비싼 이유 — 계절별 누진구간 차이 (2026년 ..."가 "(2026년" 근처에서 어색하게 끊김)다. 이건 §1 결정1 하단에 이미 남겨뒀던 회수 조건("이견 있으면 §8에서 회수 가능")이 실제로 발동한 경우다.

**확정 방향(마스터 직접 지시, 재논의 대상 아님)**: `app/blog/page.tsx` / `BlogListWithSearch.tsx`의 **목록 컨테이너(폭 + 그리드 배치)만** 오늘 이전 단일 컬럼(`max-w-3xl`, `space-y-4` 방식)으로 되돌린다. **오늘 새로 만든 페이지네이션(`BlogPagination.tsx`, `?page=N` 라우팅)과 검색(`BlogListWithSearch.tsx`의 검색 상태·매칭 로직)은 전부 그대로 유지** — 사용자가 문제 삼은 건 카드 레이아웃(그리드)이지 이 두 기능이 아니다.

### 11-1. 되돌리는 정확한 클래스

**`app/blog/page.tsx`** (65행 — 페이지 컨테이너)
```diff
- <div className="mx-auto max-w-5xl px-4 py-8 sm:px-6 sm:py-12">
+ <div className="mx-auto max-w-3xl px-4 py-8 sm:px-6 sm:py-12">
```

**`components/BlogListWithSearch.tsx`** (103행 — 결과 렌더 div)
```diff
- <div className="mt-4 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
+ <div className="mt-4 space-y-4">
```
이 `div`는 "페이지 목록" 분기와 "검색 결과" 분기(§3-6) **양쪽 모두**가 공유하는 하나의 렌더 지점이다(`results.map(...)`). 따라서 수정은 이 한 줄뿐이며, 검색 결과 화면도 자동으로 단일 컬럼이 된다 — 별도 분기 처리 불필요.

**명시적으로 변경 없음**:
| 파일 | 상태 | 비고 |
|---|---|---|
| `components/BlogPagination.tsx` | 무수정 | 자체 `flex items-center justify-center`로 폭에 의존하지 않는 컴포넌트 — 컨테이너가 5xl이든 3xl이든 렌더 동일함을 코드 확인 완료 |
| `app/blog/page.tsx`의 `searchParams`/`normalizePage`/리다이렉트/404 로직 | 무수정 | §2-2 그대로 |
| `lib/blog.ts`의 `BLOG_PAGE_SIZE`, `BlogListItem`, `toBlogListItems` | 무수정 | §11-3에서 `BLOG_PAGE_SIZE` 값 자체는 유지로 재확인 |
| `components/BlogPostCard.tsx` | 무수정(전체) | §11-2에서 재확인 — 이번 되돌림으로 이 파일은 단 한 줄도 바뀌지 않는다 |
| `app/blog/[slug]/page.tsx`(상세) | 무수정 | 이미 `max-w-3xl` — 확인 완료, 애초에 대상 아니었음 |

> 표현 정정: §6 광고 배치 문구의 "그리드 + 페이지네이션 아래"는 이제 "카드 리스트 + 페이지네이션 아래"로 읽으면 된다 — 광고 위치 규칙(페이지네이션과 24px 이상 간격, "광고" 라벨) 자체는 바뀌지 않는다.

### 11-2. 판단 1 — `line-clamp-2`(제목·요약) 그대로 유지, 변경 없음

#### 실측 — `lib/blog.ts` 22편 제목 길이(공백·기호 포함 문자 수, 직접 세어 확인)

| 제목 | 길이 |
|---|---|
| "주휴수당 조건 총정리 — 주 15시간·단시간 알바·40시간 초과 실전 케이스 (2026년)" | **50자 (최장)** |
| "겨울 전기요금이 여름보다 비싼 이유 — 계절별 누진구간 차이 (2026년 주택용 저압)" | 48자 — 마스터가 전달한 반려 예시 원문 |
| "키별 정상 체중표 — 내 키에 몇 kg이 정상일까 (대한비만학회 기준, 2026년)" | 46자 |
| "예·적금 세후이자, 과세유형이 바꾼다 — 일반·세금우대·비과세 비교 (2026년)" / "4대보험, 월급 얼마면 얼마나 떼나 — 소득구간별 근로자 부담 조견표 (2026)" | 45자 |
| 나머지 17편 | 33~42자 (중앙값 약 40자) |
| "주휴수당이란? 알바도 받는 조건·계산법 총정리 (2026년)" | 33자 (최단) |

→ 45자 이상(잠재적으로 긴 제목)은 22편 중 **5편(약 23%)**, 나머지 77%는 33~42자에 몰려 있다.

#### 폭별 2줄 수용 글자 수 (제목 `text-base sm:text-lg` 세미볼드 기준, Hangul 평균 글자폭을 폰트 크기의 0.83~1.0배로 상하한 추정)

| 구간 | 카드 내부 텍스트 폭 계산 | 2줄 수용량(추정) | 22편 최장(50자) 대비 |
|---|---|---|---|
| 데스크톱 단일 컬럼(되돌린 후, ≥768px 뷰포트) | 768 − 48(`sm:px-6`) − 32(카드 `p-4`) = **688px** | 약 76~92자 | **전부 여유 있게 안 잘림** |
| 640~767px(`sm` 경계, `max-w-3xl` 미적용 구간) | 640 − 48 − 32 = **560px** | 약 62~75자 | 여유 있음, 안 잘림 |
| 모바일 <640px(375px 기준) | 375 − 32(`px-4`) − 32(카드 `p-4`) = **311px** | 약 **39~47자** | 45자 이상 5편은 경계~초과 — 잘릴 수 있음(§11-4에서 다루듯 **이번 되돌림과 무관하게 원래부터 그랬음**) |
| (비교) 오늘 3열 그리드 데스크톱 폭 — 되돌림 대상 | (976 − 32) ÷ 3 − 32 ≈ **283px** | 약 **31~38자** | 22편 중 21편(최단 33자 제외 전부)이 폭 초과 → **이게 오늘 반려된 원인의 수치적 근거** |

#### 결론 — **`BlogPostCard.tsx` 무수정. 제목·요약 `line-clamp-2` 둘 다 그대로 둔다.**

- **제목**: 되돌린 후 640px 이상(태블릿·데스크톱, 전체 사용자의 절대다수 화면 폭)에서는 22편 전체가 2줄 안에 들어와 `line-clamp-2`가 사실상 발동하지 않는 **순수 안전장치**가 된다. 오늘 문제였던 "283px 폭"이 사라지므로 원인 자체가 없어진다.
  - 모바일(<640px)에서는 45자 이상 5편이 이론상 여전히 2줄 경계~초과일 수 있으나, **이는 그리드 도입 전(07-15 원 스펙)부터 이미 있던 상태이고 이번 되돌림으로 새로 생기거나 악화되는 게 아니다**(모바일 폭은 그리드 시절에도 지금도 동일 311px, §11-4). 잘려도 잃는 부분은 제목 끝의 "(2026년)" 연도 표기 정도로 핵심 의미 손실이 작다.
  - `line-clamp-3`로 늘리거나 완전히 제거하는 안은 **채택하지 않는다.** 데스크톱 문제는 되돌림만으로 해결되고, 모바일의 미미한 잔여 리스크 때문에 모든 카드의 제목 허용 줄 수를 늘리면 카드가 세로로 더 길어져 §11-3에서 다루는 스크롤 길이 이슈만 키운다. 지금 필요한 건 "그리드를 없애는 되돌림"이지 새 타이포 정책이 아니다.
- **요약**: 22편 실측 결과 요약문은 대체로 **95~200자**(예: "겨울 전기요금" 글 요약은 약 190자대)로, 되돌린 후 가장 넓은 폭(688px, 2줄 최대 약 90자대 수용)에서도 원문 그대로 들어가는 요약이 애초에 거의 없다. 즉 요약은 컬럼 폭과 무관하게 **07-15 원 스펙부터 처음부터 2줄에서 잘리도록 설계된 요소**이며, 오늘 지적된 문제(제목이 부자연스러운 위치에서 끊김)와는 성격이 다르다 — 잘리는 지점이 폭에 따라 달라질 뿐 "2줄에서 잘린다"는 사실 자체는 원래도 정상 동작이라 회귀가 아니다. 무수정.

> 기획팀 참고용(이번 되돌림에 필수 아님, 강제 아님): 신규 글 제목을 모바일 2줄(약 39~47자) 안에 여유 있게 넣고 싶다면 40자 안팎을 기준으로 삼을 수 있다는 참고 수치. 기존 22편을 수정하라는 요청이 아니다.

### 11-3. 판단 2 — `BLOG_PAGE_SIZE = 9` 유지, 변경 없음

- §2-1이 9를 정한 근거 3가지 중 **근거 1("`lg:grid-cols-3`에서 9=3행" 그리드 정합)은 그리드 자체가 없어지므로 이번 되돌림으로 소멸**한다 — 더 이상 유효한 근거가 아님을 여기서 명시적으로 인정한다.
- 남은 두 근거는 컬럼 수와 무관하게 그대로 유효하다:
  - **근거 2(총 페이지 수 억제)**: 22편 ÷ 9 = 3페이지 그대로. 하루 13편이 늘어난 증가 속도를 고려하면 페이지를 더 잘게 쪼개지 않는 쪽이 유리하다는 논리는 컬럼 배치와 무관.
  - **근거 3(모바일 스크롤 실측)**: 애초에 이 계산 자체가 **"375px 폭에서 카드 9장을 세로로 쌓았을 때"** 기준이었다 — 원문 그대로 재인용하면 "카드 높이 약 195px, gap 포함 약 211px, 9장 세로 스택 시 약 1,880px ≈ 4~5스크롤". 이건 처음부터 **단일 컬럼 가정의 계산**이었으므로, 되돌림으로 데스크톱도 같은 단일 컬럼이 되면 **이 수치가 데스크톱에도 그대로 적용된다**(모바일 전용이었던 값이 전 구간 값이 됨). 새로 계산할 필요 없이 기존 수치 재사용.
- **되돌림으로 실제 달라지는 것 = 데스크톱 스크롤 길이.** 오늘의 3열 그리드는 9장이 3행으로 묶여 카드 스택 높이가 약 **620px**(3행 × 195px 카드 + 2 × 16px gap)에 불과했지만, 되돌린 단일 컬럼은 9장이 그대로 세로로 쌓여 약 **1,880px**(근거3과 동일 계산) — 데스크톱만 보면 확실히 3배가량 더 길어진다.
- **그럼에도 9를 유지하는 이유**: 오늘 페이지네이션을 요청한 근본 이유는 "그리드보다 짧아야 한다"가 아니라 **"22편이 끝없이 이어져 끝이 안 보인다"**(비페이지네이션 상태, 약 4,600px, 10스크롤+)였다. 되돌린 후에도 한 페이지는 최대 9편(약 1,880px)에서 끊기고 "다음" 버튼이 뜬다 — **"끝이 있다"는 원래 요청의 핵심은 컬럼 수와 무관하게 그대로 지켜진다.** 비교 기준을 "오늘 반나절 떠 있던 그리드"로 잡으면 후퇴처럼 보이지만, 그리드는 검증된 기준선이 아니라 하루도 안 돼 반려된 시도였다 — 정당한 비교 대상은 되돌리기 직전까지의 "22편 무한 스크롤"이며, 그 기준으로는 9개/페이지가 명백한 개선이다.
- 페이지당 6개로 낮추는 안(스크롤 약 1,250px로 단축)도 검토했으나 총 페이지 수가 3→4페이지로 늘어 근거 2와 충돌하고, 이번 지시가 "재탐색이 아니라 명확한 되돌림"임을 감안해 **추가 변경 없이 9 유지**로 확정한다. 실사용에서 "페이지당 스크롤이 길다"는 피드백이 또 나오면 그때 `BLOG_PAGE_SIZE` 상수 하나만 6~8 사이로 조정(§2-1에 이미 명시된 대로 나머지 구조는 안 바뀜).

### 11-4. 판단 3 — 모바일(375px) 영향 없음 확인

- 모바일(<640px)은 오늘 그리드 도입 **전부터** `grid-cols-1`이었고, 컨테이너 패딩(`px-4`)도 그리드/리스트 버전 공통이었다. `max-w-5xl`(1024px)이든 되돌릴 `max-w-3xl`(768px)이든 375px 뷰포트보다 훨씬 커서 **두 값 모두 뷰포트 폭을 실질적으로 제한하지 않는다** — 모바일에서는 컨테이너 `max-w`가 애초에 무의미한 값이었다.
- `grid grid-cols-1 gap-4`(오늘 코드)와 되돌릴 `space-y-4`는 "완전 너비 카드를 16px 세로 간격으로 쌓기"라는 동일한 렌더 결과를 만든다 — 컬럼이 1개일 때 CSS Grid와 `space-y-*`는 시각적으로 구분 불가능하다.
- **결론: 모바일 375px 화면은 되돌리기 전/후가 완전히 동일하다.** 이 판단은 클래스 단위 코드 분석 결과이며, 이번 세션에는 라이브 화면을 직접 스크린샷/조작할 도구 접근 권한이 없어 실기기·브라우저 실측 대신 코드 분석으로 갈음했다 — 개발팀 구현 후 §11-5 체크리스트에서 실제 뷰포트로 재확인 예정.

### 11-5. 구현 후 검수 체크리스트 (§9에 추가되는 항목 — 역할 ② 예고)

- [ ] `/blog` 데스크톱(768px+)에서 카드가 3열이 아니라 **단일 컬럼**(가로 폭 꽉 채운 리스트)으로 보이는지.
- [ ] 데스크톱에서 컨테이너가 화면 전체 폭이 아니라 **가운데 정렬된 좁은 폭**(`max-w-3xl`)으로 보이는지 — `/blog/[slug]` 상세로 이동 시 좌우 정렬선이 유지되는지 함께 확인(목록↔상세 폭 통일이 원래 07-15 설계 의도, §0 표).
- [ ] 마스터가 지적한 예시 제목("주휴수당 조건 총정리 — … 케이스 (2026년)", "겨울 전기요금이 … (2026년 주택용 저압)")이 데스크톱에서 어색하게 중간에 끊기지 않고 2줄 안에 자연스럽게 들어오는지.
- [ ] 검색어를 입력했을 때 검색 결과도 자동으로 단일 컬럼으로 나오는지(같은 렌더 지점 재사용, §11-1 — 별도 분기 없이 되는 게 정상).
- [ ] 페이지네이션 UI(이전/다음/번호 세그먼트, 모바일 "N / 총페이지")가 오늘과 동일하게 그대로 동작하는지 — 이번 되돌림으로 회귀되면 안 되는 부분.
- [ ] 모바일(375px)에서 되돌리기 전(오늘 그리드 상태) 스크린샷과 되돌린 후가 실질적으로 동일한지(§11-4 예측 확인 — 여기서 차이가 발견되면 §11-4 분석을 재검토해야 함).
- [ ] 페이지당 9편 기준 한 페이지 스크롤 분량이 실사용에서 과하게 느껴지지 않는지 체감 확인(§11-3에서 "끝이 있다"는 이유로 9 유지 결정했으나, 실제로 봤을 때 다시 "길다"는 반응이 나올 가능성 있음 — 그 경우 `BLOG_PAGE_SIZE`를 6~8 사이로 낮추는 안 재검토).

### 11-6. 다른 팀 전달 사항

**개발팀**
- 되돌릴 파일은 정확히 2개, 각 1줄: `app/blog/page.tsx` 컨테이너 클래스, `components/BlogListWithSearch.tsx`의 결과 렌더 `div` 클래스(§11-1 diff 그대로 적용). 그 외 — 페이지네이션(`BlogPagination.tsx`), 검색 로직(`BlogListWithSearch.tsx`의 상태/매칭 함수), `BlogPostCard.tsx`, `lib/blog.ts`의 상수·타입 — 전부 무수정.
- `BlogPostCard.tsx`는 이번 되돌림으로 단 한 줄도 바뀌지 않는다(§11-2) — 손댈 필요 없음.
- 구현 후 §11-5 체크리스트로 디자인팀 재검수 예정.

**마스터 팀장**
- 지시하신 대로 카드 레이아웃(컨테이너 폭 + 그리드→리스트)만 단일 컬럼으로 되돌리고, 페이지네이션·검색은 그대로 유지하는 스펙으로 정리했다. line-clamp(제목·요약)와 페이지당 개수(9개) 모두 **"무변경(현행 유지)"**으로 판단 — 실측 근거는 §11-2·§11-3.
- 오늘 §1 결정1(그리드 전환)은 "기존 패턴 재사용이라 톤앤매너 변경이 아니다"라는 기준으로 사전 확인 없이 진행했는데, 결과적으로 사용자가 거부했다. 판단 기준 자체(다열 그리드가 사이트 다른 곳에도 이미 있다)는 합리적이었지만, **텍스트(특히 제목)가 주가 되는 카드에는 좁은 컬럼이 실제로 가독성 문제를 일으킬 수 있다**는 게 이번에 확인됐다. 앞으로 텍스트 위주 카드에 다열 그리드를 적용할 땐 실제 컬럼 폭 기준으로 제목 잘림 여부를 사전에 계산해 검증하겠다.
- 이번 세션에는 라이브 화면을 직접 열어볼 도구 접근 권한이 없어 전부 코드 레벨 분석(클래스·폭 계산)으로 판단했다 — 구현 후 실제 화면 검수(§11-5)는 별도로 진행 필요.
