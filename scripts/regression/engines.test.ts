...
//   · **lib/ 는 스캔하지 않는다.** 스캔 루트를 lib/ 로 넓히면 즉시 **18건 FAIL**이 난다
//     (2026-08-11 실제로 scanRoots에 "lib"을 넣고 실행해 확인 — 계산이 아니라 관측이다.
//     단위 = **금지 토큰 3종의 최장 일치 1건/출현**, 이 파일들에서는
//     행 단위 = 출현 단위로 값이 같다 — 18 출현이 서로 다른 18개 줄에 흩어져 있다.
//     🔴 **기준 = HEAD `ff4a496`, lib/ 워킹트리 clean**(티켓 #68에서 같은 판정식으로 재측정,
//     18건 그대로). 앞 회차에 "lib/blog.ts 미커밋이라 드리프트 가능"이라 적었으나 그 편집은
//     그 사이 `9b3091b`로 커밋됐다. 그 커밋은 「약 4.58%」를 품은 줄(lib/blog.ts:1498)을
//     실제로 건드렸지만 강조어 2개만 삭제해 **그 줄의 토큰 수는 1로 불변**이라 내역이 안 바뀐다
//     (diff 직독 + 전량 재계수 둘 다로 확인)).
//     내역: **티켓 #60 대상 13건** = lib/blog.ts 11(전부 「약 4.58%」) + lib/calculators.ts 2
//     (전부 「약 4.58%」). **대상 아님 5건** = lib/car-tax.ts (「약 4.58%」 4 + 「약」 없는
//     「4.58%」 1 = L41). car-tax의 5건은 전부 라벨 정의부 주석/JSDoc(L6·41·44·50·95)이라
//     **정본 문서**다 — lib을 켜려면 이 5건의 별도 예외 처리가 먼저 필요하다.
//     ⚠️ 「13」과 「5」는 단위가 다르면 안 맞는다: 「약 4.58%」만 세면 car-tax는 4다.
//   · **소스 스캔이라 렌더 결과 자체는 보지 않는다.** JSX가 그 라벨을 실제로 화면에
//     찍는지, 어느 위치에 어떤 값과 함께 찍는지는 여전히 무검증이다.
//   · **화면에 찍히는 값이 맞는 값인지는 이 스위트의 축이 아니다.** 아래가 전부 PASS한다:
//     (ㄱ) 라벨을 조건부 렌더로 **숨기는** 경우 — 소스에 식별자가 남아 있는 한 (b)는 충족된다.
//          (식별자를 한 자도 안 남기고 지우면 그때는 (b)가 FAIL한다. 아래 「(b)가 강제하는
//           성질」 참조 — FAIL의 조건은 "안 찍힘"이 아니라 "출현 0회"다.)
//     (ㄴ) **틀린 공제율을 명시적으로 적는 경우**(예: 「최대(약 3.00%)」로 인라인) — 이쪽은
//          "리터럴이 없어서"가 **아니다**. 「약 3.00%」는 명백한 리터럴이지만, 금지 토큰이
//          PREPAY_EFFECTIVE_RATE에서 런타임 파생되므로 탐지 대상이 **「현재 정답값」뿐**이라
//          정답이 아닌 값은 원리적으로 그물에 안 걸린다.
//     🔴 (ㄴ)는 티켓 #52의 **실제 라이브 결함 유형 그 자체**다(화면에 틀린 공제율이 찍혔다).
//
//        🔴🔴 (b) 소비 지점 앵커를 「교체를 막는 장치」로 읽지 마라 — 막지 않는다.
//        여기서 말하는 「(b) 소비 지점 앵커」는 아래 `usageCount >= 1` **어서션 하나**만
//        가리킨다(같은 (b) 접두사를 쓰는 스캔 루트 존재·파일 목록·import 어서션은 별개다).
//        그 어서션이 기계적으로 강제하는 성질은 **단 하나**다:
//          「components/CarTaxCalculator.tsx 소스에서 (2c) stripComments로 주석을 제거하고
//           `import … from "@/lib/car-tax"` 문을 제외한 나머지 텍스트에, 식별자
//           PREPAY_DISCOUNT_PERCENT 가 **1회 이상 출현**한다」.
//        그게 전부다. **출현 ≠ 사용 ≠ 렌더**: 출현하기만 하면 되고, 그 출현이 값으로 읽히는지
//        (죽은 코드가 아닌지), JSX가 그걸 실제로 화면에 찍는지, 찍은 값이 맞는지는 **무검증**이다.
//        실측(2026-08-11 · 티켓 #68 정정본 스위트 · 기준 HEAD `ff4a496` · 전부 직접 주입·관측):
//          - :404는 정상 사용으로 두고 :414만 「최대(약 3.00%)」로 인라인 → **511 ALL PASS**.
//            = 화면에 정답과 오답이 동시에 찍히는데 아무도 못 잡는다.
//          - :404·:414 **둘 다**를 「약 3.00%」로 갈아치우고, 값에 아무 영향이 없는 **죽은 참조**
//            한 줄만 얹어도 → **511 ALL PASS** (`npx tsc --noEmit` 도 exit 0):
//              연납 할인은 1월 신청 기준 최대(약 3.00%)이며,{false && PREPAY_DISCOUNT_PERCENT}
//            = 화면 두 지점이 **전부 오답**인데 초록이다. 그러므로 「전부 교체는 닫혔다」는
//            **거짓**이다(그 문장은 티켓 #68 R1 정정본에 있었고 QA R2가 이 한 줄로 반증했다).
//            ※ 번호 주의: R1은 당시 스스로를 「#64」로 적고 있었으나 그 번호는 동시 진행 세션이
//              선점한 별건(meta description 길이, 커밋 `9b3091b`)이다. R2에서 #68로 재배번했다.
//          - **이 어서션 하나가** FAIL하는 경우는 위 성질이 깨질 때뿐이다: 두 지점을 인라인
//            문자열로 바꾸고 죽은 참조도 두지 않으면 (주석 제외·import 제외) 출현이 0회가 되어
//            「실측 0회」로 FAIL한다(실측 확인). 다른 어서션은 각자의 이유로 따로 FAIL할 수 있다.
//        → #59 대비 티켓 #68이 실제로 개선한 것도 그 하나뿐이다: 주석 :37이 앵커를 대신
//          충족시키던 자기참조를 끊어, **「출현 0회」가 이제 진짜로 FAIL을 낸다**(#68 이전에는
//          두 지점을 다 지워도 주석 1건이 세어져 PASS였다). 그 이상은 닫지 못했다.
//     → **값의 정오(正誤)는 위 car-tax 스위트가 담당한다**: 외부 공시값 리터럴 앵커
//        (이 파일 L1237 `0.0458` · L1269 "4.58" · L1274 "약 4.58%" — 셋 다 **리터럴 인자**
//        줄이다. 티켓 #68에서 grep으로 3개 전부 재확인했다(R1 정정본의 L1275는 오프바이원
//        오기였다 — L1275는 리터럴이 아니라 어서션 **메시지 문자열**이다). 티켓 #58 「외부 공시값
//        리터럴은 금지 대상이 아니라 유일한 탐지 수단」). 값이 틀리면 거기서 FAIL난다.
//        단 그 앵커도 **정의부(lib)만** 보므로, 「정의는 정답인데 컴포넌트가 다른 값을
//        인라인」한 (ㄴ)는 두 스위트 어디에도 안 걸린다 — **열린 채로 남은 축이다.**
...
suite("연납 라벨 컴포넌트 재오염 금지", (t) => {
  ...
  // ── (2c) 소비 지점 계수기: **주석은 세지 않는다** (티켓 #68 / QA R1 FAIL-2) ──
  //   티켓 #59의 앵커는 식별자 출현을 그냥 셌다. 그런데 이 컴포넌트에는 규칙을 설명하는
  //   주석이 있어서(components/CarTaxCalculator.tsx:37 「… PREPAY_DISCOUNT_PERCENT를
  //   import해서 쓴다 …」) **진짜 사용 2지점(:404·:414)을 둘 다 지워도 usageCount=1**이
  //   되어 앵커가 초록으로 남았다(2026-08-11 실측: 둘 다 지운 상태에서 **499** ALL PASS —
  //   이 「499」는 **#59 당시 스위트 기준**의 총 assertions 수다. #68 정정본 기준은 511이므로
  //   회차가 다른 두 수를 비교하지 말 것).
  //   = #59가 추가한 주석이 자기 앵커를 영구 충족시킨 **자기참조 결함**. 그래서 계수 전에
  //   **주석을 제거한 사본**에서 센다.
  //   🔴 과잉 제거 함정: "//" 를 무조건 지우면 문자열 안의 "https://…" 뒤가 통째로 날아가
  //      진짜 사용을 놓친다. 그래서 바로 앞이 ":" 인 "//" 는 건드리지 않는다((a) 계수기 양성 2).
  //
  //   ⚠️🔴 **이 제거기는 파서가 아니다. 오류 방향은 한쪽이 아니라 양쪽이다.**
  //      「오류 방향은 사용을 놓치는 쪽뿐(=앵커가 약해지는 쪽뿐)」이라고 쓰지 마라 — 티켓 #68
  //      R1 정정본이 그렇게 썼고 QA R2가 **거짓 초록(반대 방향) 2경로**를 실증해 반증했다.
  //      아래 3경로는 2026-08-11 티켓 #68에서 전부 직접 주입해 관측한 것이다(기준 HEAD ff4a496):
  //      (가) **놓치는 쪽**(과소 계수 → 앵커가 약해짐): 문자열 리터럴 안의 "/* */" 나 ":" 가
  //           앞에 없는 "//" 를 주석으로 오인해 그 뒤를 지운다 → 뒤따르는 진짜 사용이 안 세어진다.
  //             예: const u = "a//b"; const v = PREPAY_DISCOUNT_PERCENT;   → 계수 0
  //      (나) **거짓 초록 쪽**(과대 계수 → 앵커가 코드 없이도 충족됨) — 2경로:
  //           (i) ":" 인접 "//" 예외가 **진짜 주석을 살린다.** 실사용 2지점을 지우고 이 한 줄만
  //               남겨도 **511 ALL PASS**이고 `npx tsc --noEmit` 도 exit 0이다(유효한 TS다):
  //                 const 라벨규칙://PREPAY_DISCOUNT_PERCENT 는 여기서 만들지 말 것
  //                   string = "연납";
  //           (ii) **문자열 리터럴 안의 식별자.** 제거기는 주석만 지우므로 문자열은 그대로
  //                남고, 그 안의 식별자가 사용으로 세어진다. 개발자 메모·에러 메시지·문서
  //                문자열로 **자연 발생 가능**하다(사보타주가 아니어도 생긴다). 이 한 줄만
  //                남겨도 **511 ALL PASS**:
  //                  const DOC_NOTE = "라벨은 PREPAY_DISCOUNT_PERCENT 를 쓴다";
  //   🔴 **근본 한계**: 이 계수기가 세는 것은 식별자의 **출현**이지 **사용**이 아니다. 타입 위치
  //      ·문자열 안·죽은 분기(`{false && …}`) 어디에 있든 1로 세어지고(문자열·죽은 분기는
  //      아래 (3a-3)에서 실증 · 타입 위치는 정규식이 구문 위치를 구분하지 않으므로 자명),
  //      값이 실제로 읽히는지도
  //      JSX가 화면에 찍는지도 검증하지 못한다. **이 라운드에서 이 축은 닫지 않았다**(목표가
  //      축을 넓히는 게 아니라 주석이 사실을 말하게 하는 것이었다). 정규식 제거기로는 주석·
  //      문자열·코드를 정확히 구분할 수 없으므로(중첩 템플릿 리터럴·정규식 리터럴 등), 닫으려면
  //      토크나이저 수준(TS AST)이 필요하다 — **미시도**로 남긴다.
  //      (아래 (3a-3)에 (가)·(나)(i)·(나)(ii)와 죽은 분기를 못박은 **한계 실증 대조군** 4개를
  //       뒀다. 나중에 검출기를 개선해 그 경로가 막히면 그 4개의 기대값을 함께 갱신할 것 —
  //       한계를 보호하려는 장치가 아니라 한계가 실재함을 기계적으로 증명해 두는 장치다.)
  //      (4)의 리터럴 스캔은 이 제거를 쓰지 않고 주석까지 그대로 훑는다 — 주석에 값을 적는
  //      것도 금지이고, 주석 배제 규칙을 만들면 주석 위장 오염이 열리기 때문이다.
  const stripComments = (src: string): string =>
    src
      .replace(/\/\*[\s\S]*?\*\//g, " ") // 블록 주석
      .replace(/(^|[^:])\/\/[^\n]*/gm, "$1"); // 줄 주석 (":" 바로 뒤의 "//" = URL은 보존)
  const identRe = (): RegExp =>
    new RegExp(`(?<![A-Za-z0-9_$])${LABEL_IDENT}(?![A-Za-z0-9_$])`, "g");
  const countIdent = (text: string): number => text.match(identRe())?.length ?? 0;
  const countUsage = (text: string): number => countIdent(stripComments(text));
  ...
  // ── (3a-2) 소비 지점 계수기 자기시험: 대조군 (티켓 #68) ────────────────────
  //   「주석 속 식별자는 미계수 / 코드 속 식별자는 계수 / URL 든 줄의 코드 사용도 계수」
  //   세 축을 양성·음성으로 못박는다. 대조군 비공허(주석 제거 **전**에는 1회로 세어진다)를
  //   함께 둬 이 어서션들이 공허하지 않음을 증명한다 — 그게 #59 결함의 재현이기도 하다.
  const cLine = `// 라벨은 lib/car-tax.ts의 ${LABEL_IDENT} 를 import해서 쓴다`;
  const cBlock = `/**\n * 라벨(${LABEL_IDENT})은 여기서 만들지 말 것.\n */\nconst x = 1;`;
  t.eq(countUsage(cLine), 0, "(a) 계수기 음성 1: 줄 주석 속 식별자는 사용으로 세지 않는다");
  t.eq(countUsage(cBlock), 0, "(a) 계수기 음성 1: 블록 주석 속 식별자는 사용으로 세지 않는다");
  t.eq(
    countIdent(cLine),
    1,
    "(a) 계수기 대조군 비공허: 주석 제거 전에는 줄 주석이 1회로 세어진다(#59 결함 재현)"
  );
  t.eq(
    countIdent(cBlock),
    1,
    "(a) 계수기 대조군 비공허: 주석 제거 전에는 블록 주석도 1회로 세어진다"
  );
  t.eq(
    countUsage(`// 출처 https://www.wetax.go.kr 참고 — ${LABEL_IDENT} 를 쓸 것`),
    0,
    "(a) 계수기 음성 2: 주석 줄 안의 URL 뒤 식별자도 미계수(줄 시작 \"//\"가 우선)"
  );
  t.eq(
    countUsage(`label={\`(\${${LABEL_IDENT}})\`}`),
    1,
    "(a) 계수기 양성 1: JSX 보간 사용은 1회로 센다"
  );
  t.eq(
    countUsage(`const src = "https://www.wetax.go.kr"; const v = ${LABEL_IDENT};`),
    1,
    "(a) 계수기 양성 2: URL(https://)이 같은 줄에 있어도 코드 사용을 놓치지 않는다(과잉 제거 방지)"
  );
  t.eq(
    countUsage(`const v = ${LABEL_IDENT}; // ${LABEL_IDENT} 는 여기서 만들지 말 것`),
    1,
    "(a) 계수기 양성 3: 같은 줄의 코드 사용 1 + 주석 언급 1 → 1회"
  );

  // ── (3a-3) **한계 실증 대조군** (티켓 #68 / QA R2 FAIL-2·D-2) ──────────────
  //   아래 4개는 "옳은 동작"이 아니라 **계수기가 못 하는 일**을 기대값으로 못박은 것이다.
  //   위 (2c) 주석의 (가)·(나)(i)·(나)(ii)·「출현≠사용」주장이 산문이 아니라 실제 동작임을
  //   여기서 기계적으로 증명한다. 🔴 검출기를 개선해 이 경로들을 막게 되면 이 4개의 기대값도
  //   함께 갱신할 것(한계를 보호하는 장치가 아니라, 한계가 실재함을 증명하는 장치다).
  t.eq(
    countUsage(`const u = "a//b"; const v = ${LABEL_IDENT};`),
    0,
    "(a) 한계 (가): 문자열 안의 \"//\"를 주석으로 오인해 **진짜 사용을 놓친다**(과소 계수)"
  );
  t.eq(
    countUsage(`const 라벨규칙://${LABEL_IDENT} 는 여기서 만들지 말 것\n  string = "연납";`),
    1,
    "(a) 한계 (나)(i): \":\" 인접 \"//\" 예외가 **진짜 주석을 사용으로 센다**(거짓 초록·과대 계수)"
  );
  t.eq(
    countUsage(`const DOC_NOTE = "라벨은 ${LABEL_IDENT} 를 쓴다";`),
    1,
    "(a) 한계 (나)(ii): **문자열 리터럴 속 식별자**를 사용으로 센다(거짓 초록·자연 발생 가능)"
  );
  t.eq(
    countUsage(`<p>최대(약 3.00%)이며,{false && ${LABEL_IDENT}}</p>`),
    1,
    "(a) 한계 (출현≠사용): **죽은 분기의 참조**도 1회로 센다 — 화면 값이 오답이어도 앵커는 충족"
  );
  ...
  // 소비 지점 앵커 — 판정식은 **이 한 줄이 전부**다:
  //   「(2c) stripComments로 주석 제거 → import 문 제외 → 남은 텍스트에 식별자 1회 이상 **출현**」.
  //   🔴 **출현이지 사용이 아니다**: 문자열 리터럴 안·죽은 분기(`{false && …}`) 안의 참조도
  //      1회로 세어져 이 앵커를 충족시킨다(위 (2c) 「근본 한계」 + (3a-3) 한계 실증 대조군).
  //      "화면에 라벨이 찍힌다"·"라벨 값이 맞다"는 여기서 **아무것도 보장되지 않는다.**
  //   (출현 횟수는 숫자로 못박지 않는다 — 정당한 카피 변경에 썩는다. "1회 이상"만 잠근다.
  //    2026-08-11 실측 2회 = :404 Tier③ 라벨 · :414 하단 안내. 주석 :37은 이제 안 세어진다.)
  ...
  const usageCount = countUsage(bodyAfterImport);
  t.ok(
    usageCount >= 1,
    `(b) ${CALC} 의 import 문 밖·주석 제외 텍스트에 ${LABEL_IDENT} 가 1회 이상 **출현** (실측 ${usageCount}회)` +
      "\n  ※ 출현만 본다 — 실제 사용·렌더·값의 정오는 이 앵커의 보장 범위가 아니다."
  );
